...
export class PrintInvoice extends React.PureComponent {
    render() {
      ...
      return (
          <>
          {this.props.companyInfo.map((c, i) => (
              <div key={i}>
              ...
              <table className="textScale2 table table-bordered docNumber">
                ...
                <tbody>
                  <td></td>
                  <td>{new Date(this.props.order.createdAt).toLocaleString()}</td>
                </tbody>
              </table>
                    ...
                    {this.props.order.products.map((p, i) => (
                        <tr key={i}>
                          <td>number</td>
                            ...
                            <td>
                              {this.props.order.paymentIntent.withNDS}
                            </td> 
                            <td>
                              {this.props.order.paymentIntent.nds}
                            </td> 
                        </tr>
                    ))}
                </tbody>
